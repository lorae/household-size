...
## Case 1: a plain-English explanation of what this type of move is

**Figure 1.3: $m_{k+1} = m_k - 1$**
```{r, fig.pos = "H", out.width="100%", echo = FALSE}
knitr::include_graphics("proof_files/figure1-3.png")
```
\captionof{figure}{An individual is moved from household $k$, with $m_k = 4$ members, to household $k+1$, with $m_{k+1} = 3$ members. This results in a configuration where $m_k' = 3$ and $m_{k+1}' = 4$.}
...
In this particular case, the initial number of occupants of household $k$ is exactly one larger than the initial number of occupants in household $k+1$. With such a shift, we anticipate no change in the average person-level household size, because the composition of the households remains the same: in essence, it is just the numbering that changes. The algebra bears this out:
