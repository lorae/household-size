...
Note: In this form of the equation, we've added an extra household, household $j = H + 1$, purely as a useful convention for reasons that will soon be apparent. This $H+1$th household can be regarded as a household of 0 people. In fact, the sum can be interpreted as the squared household size of the first $H$ households plus a potentially infinite number of 0-person households.

Any change in one configuration among the $H$ households can be made by incrementally moving one individual at a time from one household to another. [expand upon what we mean in a picture, perhaps. Or just explain it better]. Suppose we move one individual from household $j = k$ to household $j = k+1$. Then mean household size changes from:
